...
`assets/js/main.js`의 전화번호와 상담예약하기 주소를 실제 값으로 교체해야 합니다.
...
- 지역 페이지는 `/region/상위지역/시군구/` 형태를 기본으로 운영합니다.
...
## 테이블오더 롱폼 이미지 분리
- 테이블오더 장비별 페이지에서 세로형 롱폼 이미지를 예시 갤러리로 사용하지 않도록 제거했습니다.
- `/equipment/table-order/info/` 페이지에서 롱폼 이미지를 한 페이지형 콘텐츠로 표시합니다.
